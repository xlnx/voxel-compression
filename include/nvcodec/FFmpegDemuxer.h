...
private:
    ...
    */
    FFmpegDemuxer(AVFormatContext *fmtc) : fmtc(fmtc) {
        if (!fmtc) {
			vm::eprintln("ERROR: No AVFormatContext provided.");
            return;
        }
        ...
        case AV_PIX_FMT_YUVJ444P:   // jpeg decoder output is subsampled to NV12 for 422/444 so treat it as 420
            ...
            break;
        default:
			vm::eprintln("WARNING: ChromaFormat not recognized. Assuming 420");
            nBitDepth = 8;
            nChromaHeight = (nHeight + 1) >> 1;
            nBPP = 1;
        }
        ...
        if (bMp4H264) {
            ...
            if (!bsf) {
				vm::eprintln("ERROR: FFmpeg error: {} {} {}", __FILE__, __LINE__, "av_bsf_get_by_name() failed");
                return;
            }
    ...
    AVFormatContext *CreateFormatContext(DataProvider *pDataProvider) {
        ...
        if (!(ctx = avformat_alloc_context())) {
			vm::eprintln("ERROR: FFmpeg error: {} {}", __FILE__, __LINE__);
            return NULL;
        }
